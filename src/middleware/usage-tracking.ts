/**
 * Usage Tracking Middleware for ProseCreator Marketing
 *
...
const SERVICE_NAME = 'prosecreator-marketing';
...
async function flushBatch(): Promise<void> {
  ...
  logger.debug(`Flushing usage batch: ${batch.length} reports`);
...
function detectOperation(req: Request): string {
  ...
  if (path.includes('/campaigns')) {
    if (path.includes('/create') || method === 'POST') return 'campaign_create';
    if (path.includes('/update') || method === 'PUT' || method === 'PATCH') return 'campaign_update';
    if (path.includes('/delete') || method === 'DELETE') return 'campaign_delete';
    if (path.includes('/launch')) return 'campaign_launch';
    if (path.includes('/pause')) return 'campaign_pause';
    if (path.includes('/schedule')) return 'campaign_schedule';
    if (path.includes('/stats')) return 'campaign_stats';
    return 'campaign_read';
  }

  if (path.includes('/ads')) {
    if (path.includes('/create') || method === 'POST') return 'ad_create';
    if (path.includes('/update') || method === 'PUT' || method === 'PATCH') return 'ad_update';
    if (path.includes('/delete') || method === 'DELETE') return 'ad_delete';
    if (path.includes('/generate')) return 'ad_generate';
    if (path.includes('/optimize')) return 'ad_optimize';
    if (path.includes('/ab-test')) return 'ad_ab_test';
    if (path.includes('/performance')) return 'ad_performance';
    return 'ad_read';
  }

  if (path.includes('/email')) {
    if (path.includes('/send')) return 'email_send';
    if (path.includes('/template')) return 'email_template';
    if (path.includes('/schedule')) return 'email_schedule';
    if (path.includes('/campaign')) return 'email_campaign';
    if (path.includes('/automation')) return 'email_automation';
    if (path.includes('/list')) return 'email_list_management';
    if (path.includes('/stats') || path.includes('/analytics')) return 'email_analytics';
    return 'email_operation';
  }

  if (path.includes('/social')) {
    if (path.includes('/post')) return 'social_post';
    if (path.includes('/schedule')) return 'social_schedule';
    if (path.includes('/connect') || path.includes('/auth')) return 'social_connect';
    if (path.includes('/analytics')) return 'social_analytics';
    if (path.includes('/engagement')) return 'social_engagement';
    if (path.includes('/generate')) return 'social_content_generate';
    return 'social_operation';
  }

  if (path.includes('/analytics')) {
    if (path.includes('/dashboard')) return 'analytics_dashboard';
    if (path.includes('/report')) return 'analytics_report';
    if (path.includes('/export')) return 'analytics_export';
    if (path.includes('/roi')) return 'analytics_roi';
    if (path.includes('/conversion')) return 'analytics_conversion';
    if (path.includes('/audience')) return 'analytics_audience';
    if (path.includes('/trends')) return 'analytics_trends';
...
async function sendUsageReport(report: UsageReport, retryCount = 0): Promise<void> {
  ...
  try {
    ...
    if (response.status === 204 || response.ok) {
      logger.debug('[Usage Tracking] Report sent', {
    ...
    logger.warn('Usage tracking error response', {
    ...
    if (!errorMessage.includes('abort')) {
      logger.warn('Failed to send usage report', { error: errorMessage });
    }
...
export function usageTrackingMiddleware(
  ...
  res.on('finish', () => {
    ...
    const report: UsageReport = {
      ...
      requestId: req.tenantContext?.requestId || extractHeader(req, 'x-request-id') || req.id,
    ...
    logger.debug('[Usage Tracking] Sending report', {
      operation,
      userId,
    ...
    if (USAGE_TRACKING_CONFIG.enableBatching) {
      ...
      sendUsageReport(report).catch((err) => {
        logger.error('[Usage Tracking] Unexpected error', {
